...
| Contrato e fronteira do planejador LangChain4j | IMPLEMENTADO LOCALMENTE, NÃO CONECTADO À FILA | `AuthoringPlanContract` define saída JSON fechada e envia ao modelo somente objetivo, faixa, duração, modo, palavra, componentes, estímulos e tema; IDs de mídia/consentimento e notas livres ficam fora. `AuthoringPlanService` chama provedor só após recuperar fonte aprovada e no escopo; testes com provedor falso rejeitam citação inventada, palavra alterada, mecânica fora do pedido, campo extra, JSON concatenado e linguagem punitiva | configurar provedor docente permitido, persistir plano, executar avaliação e ligar worker/Estúdio |
