...
| Contrato e fronteira do planejador LangChain4j | IMPLEMENTADO LOCALMENTE, SEM FONTE APROVADA EM PRODUÇÃO | `AuthoringPlanContract` define saída JSON fechada e envia ao modelo somente objetivo, faixa, duração, modo, palavra, componentes, estímulos e tema; IDs de mídia/consentimento e notas livres ficam fora. O segundo worker usa Ollama opt-in, requer modelo explícito, só chama após recuperar fonte aprovada e grava plano com hash. [Ensaio sintético local](AUTHORING_MODEL_SMOKE.md) expôs falhas de formato em modelos menores, sem validar didática nem Oracle | curadoria real de fontes, avaliação docente do modelo e conexão do plano ao Estúdio |
...
- o plano é um rascunho não executável; somente `LearningStoryPack` aprovado e validado pode chegar
  ao tablet. Houve apenas smoke sintético de modelos locais; nenhuma fonte de produção aprovada,
  geração de imagem ou executor Codex foi acionado neste corte.
