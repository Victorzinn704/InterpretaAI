...
6. `POST /api/v2/assignments/{assignmentId}/revoke` retira uma atribuição ativa de forma idempotente e registra auditoria uma única vez.
7. Na sincronização seguinte, o tablet remove o vínculo retirado e revoga a sessão ativa. Bytes imutáveis compartilhados por outras atribuições permanecem no cache.
...
| Retirada | atribuição revogada no servidor | Retirada da turma |
| Sincronização | vínculo removido no tablet | Retirada do aparelho |
| Execução | eventos recebidos | Aula iniciada |
| Evidência | lote aceito | Participação registrada |

“Publicada”, “enviada”, “pronta” e “retirada” são estados diferentes. A tela mostra o último estado confirmado pela fonte responsável. Uma retirada no servidor bloqueia novos downloads e confirmações imediatamente; aparelhos offline recebem a mudança na próxima sincronização.
