...
| Aprovação e publicação de versão imutável | IMPLEMENTADO LOCALMENTE | `story_version` nunca atualiza JSON/hash; rota adulta exige escola e autoria, controla revisão/idempotência e audita aprovação/publicação; `StoryVersionControllerTest` cobre transições e negações | ligar a saída validada do worker à criação do rascunho e publicar atribuições/manifesto |
