...
| Validação, aprovação e publicação de versão imutável | PARCIAL — NÃO LIBERAR AUTORIA GERADA | o validador Java, a vinculação de mídia privada e as transições auditadas existem localmente; porém o contrato do pacote exige `approvedBy`, `approvedAt` e `reviewedByTeacher=true` já no ingresso do rascunho, e os testes preenchem esses campos antes da revisão humana | separar rascunho de pacote entregável conforme [ADR 004](adr/004-draft-vs-deliverable-pack.md), validar revisão real e só então ligar a geração automática |
