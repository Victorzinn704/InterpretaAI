...
| Fonte local de pacote e catálogo | IMPLEMENTADO LOCALMENTE | Room v2 guarda pacote imutável, variantes e atribuição por aparelho/prioridade/expiração; migração 1→2 passou em teste instrumentado | cota real de armazenamento e retenção do cache |
...
| Prontidão da jornada | IMPLEMENTADO LOCALMENTE | a Home só mostra atribuição do aparelho vigente e `FULLY_CACHED`; antes de abrir, revalida arquivos e hashes no armazenamento privado | medir reinício/reconexão e retomar sessão após morte do processo |
...
| Renderer `COMIC`, `PUZZLE`, `WORD_BUILDER`, dupla e fim por dados | IMPLEMENTADO LOCALMENTE | a Home abre pacote preparado; teste instrumentado percorre quadrinho, puzzle por toque e arraste, letras, conversa em dupla e fim em 360×640, 412×915 e 800×1280; [capturas](../../output/screenshots/story-pack/) | exercitar publicação/download reais, criar caso `BOLA` por dados, testar áudio/microfone e revisar com professora/crianças |
