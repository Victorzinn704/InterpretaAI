...
2. **[2026-09-15] Measure response phases; do not equate connection speed with child-perceived latency**
   Do instead: compare Gemini 3.8 Flash and Mistral with separate ACK, validated-text and complete-audio p50/p95; keep `maxRetries(0)` in every provider, one idempotent Android retry inside the six-second deadline, and never stream unvalidated model tokens to the child.
